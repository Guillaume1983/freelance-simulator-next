          ...
          {rows.map((row, idx) => (
            <React.Fragment key={idx}>
              <tr className={`group transition-colors ${row.highlight ? 'bg-slate-50/50 dark:bg-slate-900/30 font-bold' : ''} ${row.isFinal ? 'bg-indigo-50/30 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 font-black' : ''}`}>
                ...
                {regimes.map((r: any) => {
                  // Correction de la logique de calcul pour éviter l'erreur TypeScript
                  const rawValue = r[row.key] as number;
                  const displayValue = (row.key === 'net' || row.key === 'beforeTax') ? rawValue / 12 : rawValue;
                  
                  return (
                    <td key={r.id} className={`p-4 text-center font-bold transition-all duration-300 ${selectedId === r.id ? 'col-active' : ''} ${row.isFinal ? 'text-lg' : 'text-sm'} ${row.color || ''}`}>
